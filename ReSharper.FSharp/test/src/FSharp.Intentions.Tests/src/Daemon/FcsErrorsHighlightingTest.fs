...
type FcsErrorsHighlightingTest() =
    ...
    [<TestFileExtension(FSharpScriptProjectFileType.FsxExtension)>]
    [<Test; Explicit>] member x.``Script - Syntax - Directive 01``() = x.DoNamedTest()
    ...
    [<Test>] member x.``Binding in namespace 01``() = x.DoNamedTest()
    [<Test; FSharpSignatureTest>] member x.``Binding in namespace 02 - val``() = x.DoNamedTest()
    ...
    [<Test>] member x.``XmlDoc - Missing parameter``() = x.DoNamedTest()
