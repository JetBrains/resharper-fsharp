...
namespace JetBrains.ReSharper.Plugins.FSharp.Psi.Impl.DocComments
{
  internal class FSharpDocCommentXmlPsi : ClrDocCommentXmlPsi<XmlDocBlock>
  {
    ...
    private static IEnumerable<string> CommentNodeToLines([NotNull] DocComment commentNode)
    {
    ...
    [Pure, NotNull]
    private static IEnumerable<int> GetMultilineCommentLinesStartOffsets([NotNull] DocComment commentNode,
      bool isShifted)
    {
      var commentText = commentNode.CommentText;
    ...
    {
      var comment = (DocComment) commentNode;
