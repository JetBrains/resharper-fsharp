...
namespace JetBrains.ReSharper.Plugins.FSharp.Services.Formatter
{
  ...
  public class FSharpCodeFormatter : CodeFormatterBase<FSharpFormatSettingsKey>
  {
    ...
    public override MinimalSeparatorType GetMinimalSeparatorByNodeTypes(TokenNodeType leftTokenType, TokenNodeType rightTokenType)
    {
      ...
      if (glueingCache.GetOrAdd(new FormatterImplHelper.TokenTypePair(leftTokenType, rightTokenType), IsTokensGlued))
      {
    ...
    public override bool IsNewLine(ITreeNode ws) =>ws is IWhitespaceNode node && node.IsNewLine;
    ...
    {
      if (parent is IFSharpFile && IsNewLine(nextNode))
        return;
    ...
    private static bool IsTokensGlued(FormatterImplHelper.TokenTypePair key)
    {
