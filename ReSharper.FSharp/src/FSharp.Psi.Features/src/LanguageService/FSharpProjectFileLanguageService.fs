...
type FSharpProjectFileLanguageService(projectFileType, fsCheckerService: FSharpCheckerService) =
    ...
    override x.GetPsiProperties(projectFile, sourceFile, _) =
        FSharpPsiProperties(projectFile, sourceFile) :> _

[<ProjectFileType(typeof<FSharpScriptProjectFileType>)>]
type FSharpScriptProjectFileLanguageService(projectFileType, fsCheckerService) =
    inherit FSharpProjectFileLanguageService(projectFileType, fsCheckerService)
