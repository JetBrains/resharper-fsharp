...
type FsiOptions() =
    ...
    static member GetValueProperty(lifetime, settings: IContextBoundSettingsStoreLive, getter: Expression<Func<FsiOptions,_>>) =
...
type FsiOptionsProvider(lifetime: Lifetime, solution: ISolution, settingsStore: ISettingsStore) =
    ...
    member val UseAnyCpuVersion     = FsiOptions.GetValueProperty(lifetime, store, fun s -> s.UseAnyCpuVersion)
    member val ShadowCopyReferences = FsiOptions.GetValueProperty(lifetime, store, fun s -> s.ShadowCopyReferences)
    member val FsiArgs              = FsiOptions.GetValueProperty(lifetime, store, fun s -> s.FsiArgs)
    member val MoveCaretOnSendLine  = FsiOptions.GetValueProperty(lifetime, store, fun s -> s.MoveCaretOnSendLine)
    member val ExecuteRecents       = FsiOptions.GetValueProperty(lifetime, store, fun s -> s.ExecuteRecents)
    member val FsiPath              = FsiOptions.GetValueProperty(lifetime, store, fun s -> s.FsiPath)
