...
type internal FSharpSigTreeBuilder(sourceFile, lexer, sigs, lifetime, path) =
    ...
    member x.ProcessModuleMemberSignature(moduleMember) =
        ...
        | SynModuleSigDecl.NestedModule(SynComponentInfo(attrs, _, _, _, XmlDoc xmlDoc, _, _, _), _, _, memberSigs, range) ->
    ...
    member x.ProcessTypeMemberSignature(memberSig) =
        ...
        | SynMemberSig.Member(SynValSig(attrs, _, _, synType, arity, _, _, XmlDoc xmlDoc, _, _, _), flags, range) ->
            let mark = x.MarkAndProcessIntro(attrs, xmlDoc, null, range)
            x.ProcessReturnTypeInfo(arity, synType)
            let elementType =
