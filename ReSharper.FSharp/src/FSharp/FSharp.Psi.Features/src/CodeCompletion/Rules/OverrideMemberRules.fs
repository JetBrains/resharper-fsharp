...
module OverrideRuleModule =    
    ...
    let getMemberOwnerAndItsMembers (context: FSharpCodeCompletionContext) (generatorContext: FSharpGeneratorContext) : ITreeNode * ITypeBodyMemberDeclaration seq =
        ...
        let (|IsInInterfaceImplScope|_|) (memberDecl: IMemberDeclaration option) (impl: IInterfaceImplementation) =
            let isInScope =
               isNotNull impl &&
               let interfaceKeyword = impl.InterfaceKeyword
               let indent = impl.Indent
               isNotNull interfaceKeyword &&
               caretColumn > indent &&
               (memberDecl.IsNone || memberDecl.Value.Indent > indent)
    ...
    let mayGenerateOverrides (context: FSharpCodeCompletionContext) (generatorContext: FSharpGeneratorContext) (node: ITreeNode) =
        let caretCoords = getCaretCoords context
        ...
        | TokenType FSharpTokenType.DOT _ -> 
            ...
            let memberDecl =
                ...
                | _ ->
                    anchor.GetContainingNode<IMemberDeclaration>()

            isNotNull memberDecl && isNotNull memberOwner &&
    ...
    let addOverrideItems
        ...
        let generatorElements =
            getOverridableElements generatorContext
...
type OverrideMemberRule() =
    ...
    override this.IsAvailable(context) =
        context |> isOverrideRuleAvailable isWhitespace (fun owner -> not (owner :? IInterfaceImplementation))

    override this.AddLookupItems(context, collector) =
       addOverrideItems context collector
...
type QualifiedOverrideRule() =
    ...
    override this.IsAvailable(context) =
        context |> isOverrideRuleAvailable isDot (fun owner -> not (owner :? IInterfaceImplementation))
