...
namespace JetBrains.ReSharper.Daemon.FSharp.Stages
{
  ...
  public class FSharpIdentifierTooltipProvider : IdentifierTooltipProvider<FSharpLanguage>
  {
    ...
    {
      if (!ShouldShowTooltip(highlighter))
        return string.Empty;
      ...
      // todo: implement for long identifier elements and declarations
      ...
      var tokenCoords = document.GetCoordsByOffset(highlighter.Range.EndOffset);
      var names = ListModule.OfArray(new[] {token.GetText()}); // todo: provide qualifiers
      var getTooltipAsync = checkResults.GetToolTipTextAlternate((int) tokenCoords.Line.Plus1(),
        (int) tokenCoords.Column, string.Empty, names, FSharpTokenTag.Identifier); // todo: tag.String for fsx
    ...
    public static string GetXmlDocText(FSharpXmlDoc xmlDoc)
    {
      ...
      if (xmlDoc.IsText)
        return ((FSharpXmlDoc.Text) xmlDoc).Item;
